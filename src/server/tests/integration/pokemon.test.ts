...
describe("Route pokemons", () => {
  ...
  test("should return all pokemons type normal and legendary", async () => {
    ...
    const secondReponse = await request(`pokemon/?type=${type}`).then(res => {
      expect(res.status).toEqual(200);
      expect(res.data.length).toBeGreaterThan(1);
    });
  });

  test("should return all legendary pokemons", async () => {
    const response = await request("legendary").then(res => {
      expect(res.status).toEqual(200);
      expect(res.data.length).toBeGreaterThan(1);
    });
  });
...
describe("Pagination", () => {
  test("should return list with pagination", async () => {
    ...
    const response = await request(`pokemon/page/${page}`).then(res => {
      expect(res.status).toEqual(200);
